...
const Wallets = require('../model/account');
const Transactions = require('../model/transacation');
...
const debitAccount = async ({amount, number, purpose, reference, summary, trnxSummary, session}) => {
  ...
  const updatedWallet = await wallet.findOneAndUpdate({number}, { $inc: { balance: -amount } }, {session});
  const transaction = await Transactions.create([{
    trnxType: 'DR',
    purpose,
    amount,
